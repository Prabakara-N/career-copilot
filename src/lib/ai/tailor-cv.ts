...
import { renderTailoredCvPdf, type RenderedCv } from "@/lib/cv/render-pdf";
import {
  TAILOR_CV_SYSTEM,
  TailoredCopySchema,
  buildTailorCvPrompt,
...
export type TailoredCv = RenderedCv & { atsScore: AtsScore };

export async function generateTailoredCvData(args: {
  company: string;
  role: string;
  jdText: string;
...
export async function tailorCvForJd(args: {
  company: string;
  role: string;
  jdText: string;
  seedKeywords?: readonly string[];
}): Promise<TailoredCv> {
  const { cvData } = await generateTailoredCvData(args);
